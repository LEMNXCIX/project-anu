use simplelog::{
    ConfigBuilder, LevelFilter, WriteLogger, TermLogger, CombinedLogger, TerminalMode, ColorChoice,
};
...
pub fn initialize_logger() -> Result<(), std::io::Error> {
    ...
    let config_dir = dirs::config_dir()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "No se pudo obtener el directorio de configuración"))?;
    ...
    ]).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("Error al inicializar CombinedLogger: {}", e)))?;

    info!("Logger configurado, escribiendo en: {:?}", log_dir.join("app.log"));
    Ok(())
}